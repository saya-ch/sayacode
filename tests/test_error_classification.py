...
背景：`_classify_error` 的分类直接决定恢复动作。曾出现两类误判：
...
def test_context_overflow_is_prompt_too_long(message):
    ...
    assert _classify_error(message) == "prompt_too_long"
...
def test_output_limit_is_max_output_tokens(message):
    ...
    assert _classify_error(message) == "max_output_tokens"
...
def test_transient_errors_are_recoverable(message):
    assert _classify_error(message) == "recoverable"
...
def test_parameter_errors_are_fatal_not_retried(message):
    ...
    assert _classify_error(message) == "fatal"
...
def test_prompt_too_long_patterns_are_checked_before_output_patterns():
    ...
    assert _classify_error(combined) == "prompt_too_long"


def test_pattern_tables_do_not_overlap_on_context_overflow():
    """max_output 表不应包含上下文超限的关键字。"""
    for pattern in _MAX_OUTPUT_TOKENS_PATTERNS:
        assert "context" not in pattern
    assert any("context" in pattern for pattern in _PROMPT_TOO_LONG_PATTERNS)


def test_recoverable_patterns_include_spaced_timed_out():
    """'timeout' 匹配不到 'timed out'，两种写法都要覆盖。"""
    assert "timed out" in _RECOVERABLE_ERROR_PATTERNS


def test_retry_delay_is_exponential():
    delays = [_retry_delay(attempt) for attempt in range(1, 4)]

...
async def test_chat_completions_protocol_uses_chat_endpoint() -> None:
    ...
    with local_provider([WireReply(completion)]) as (url, requests):
        model = AgentRuntime._model_for(
            profile("openai_chat_completions", url + "/custom/v1")
        )
...
async def test_keyless_custom_endpoint_never_receives_ambient_openai_key(
    ...
    completion = {
        "id": "chatcmpl-keyless", "object": "chat.completion", "created": 1,
        "model": "wire-model",
    ...
    with local_provider([WireReply(completion)]) as (url, requests):
        model = AgentRuntime._model_for(
            profile("openai_chat_completions", url + "/v1", api_key=None)
        )
...
async def test_responses_api_tool_roundtrip_uses_responses_wire_format() -> None:
        ...
        [
            {
                ...
                "status": "completed",
                "content": [
                    {"type": "output_text", "text": "Found main", "annotations": []}
                ],
            }
        ],
        number=2,
    ...
    with local_provider([WireReply(first), WireReply(second)]) as (url, requests):
        model = AgentRuntime._model_for(profile("openai_responses", url + "/custom/v1"))
        assert isinstance(model, ChatOpenAI)
...
async def test_responses_api_sse_emits_text_and_usage() -> None:
    ...
    stream = (
        sse("response.created", {"response": {**finished, "output": [], "status": "in_progress", "usage": None}})
        + sse(
    ...
    assert any(chunk.usage_metadata and chunk.usage_metadata["total_tokens"] == 6 for chunk in chunks)
...
async def test_anthropic_messages_tool_roundtrip_uses_native_blocks() -> None:
    ...
    first = anthropic_message(
        [{"type": "tool_use", "id": "toolu_wire", "name": "lookup_symbol", "input": {"name": "main"}}]
    )
    second = anthropic_message([{"type": "text", "text": "Found main"}], number=2)
    with local_provider([WireReply(first), WireReply(second)]) as (url, requests):
        model = AgentRuntime._model_for(profile("anthropic_messages", url))
...
async def test_anthropic_messages_sse_emits_text_and_usage() -> None:
    initial = {**anthropic_message([{"type": "text", "text": ""}]), "content": [], "stop_reason": None}
    ...
    stream = (
        ...
        + sse("message_delta", {"delta": {"stop_reason": "end_turn", "stop_sequence": None}, "usage": {"output_tokens": 2}})
        + sse("message_stop", {})
    )
    with local_provider([WireReply(stream, "text/event-stream")]) as (url, requests):
        model = AgentRuntime._model_for(profile("anthropic_messages", url))
    ...
    assert any(chunk.usage_metadata and chunk.usage_metadata["output_tokens"] == 2 for chunk in chunks)
...
async def test_gemini_native_tool_roundtrip_uses_generate_content() -> None:
    ...
    with local_provider([WireReply(first), WireReply(second)]) as (url, requests):
        model = AgentRuntime._model_for(profile("gemini_generate_content", url))
...
async def test_ollama_native_tool_roundtrip_uses_api_chat(
    ...
    with local_provider(replies) as (url, requests):
        model = AgentRuntime._model_for(profile("ollama_native_chat", url, api_key=None))
...
async def test_ollama_native_uses_explicit_api_key_for_authenticated_endpoint() -> None:
    ...
    with local_provider([reply]) as (url, requests):
        model = AgentRuntime._model_for(profile("ollama_native_chat", url))
        answer = await model.ainvoke("ping")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "anthropic_messages",
            {"type": "error", "error": {"type": "authentication_error", "message": "invalid test key"}},

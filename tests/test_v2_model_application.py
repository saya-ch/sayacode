...
from sayacode.app import SayacodeApp, create_app
from sayacode.cli import amain, build_parser
    ...
    def _generate(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        if "sayacode_capability_probe" in prompt:
            answer = AIMessage(
                content="",
                tool_calls=[{
                    "name": "sayacode_capability_probe", "args": {"value": "ping"},
                    "id": "probe-1", "type": "tool_call",
                }],
            )
        else:
...
async def test_six_field_one_run_profile_and_partial_rejection(
    ...
    incomplete = build_parser().parse_args([
        "--workspace", str(tmp_path), "--protocol", "openai_responses",
    ])
...
async def test_named_profile_test_ignores_one_run_model_override(tmp_path: Path) -> None:
    ...
    app.runtime._model_for = model_for  # type: ignore[method-assign]
    try:
...
async def test_invalid_saved_model_config_exits_as_config_error(
    ...
    code = await amain([
        "--workspace", str(tmp_path), "-p", "hello", "--output-format", "json"
    ])

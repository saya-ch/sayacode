    ...
    def _generate(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        answer = (
            AIMessage(content="", tool_calls=[
                {"name": "mcp__working_directory", "args": {},
                 "id": "stdio-call", "type": "tool_call"}
            ])
            if self.calls == 1 else AIMessage(content="MCP complete")
        )
...
async def test_real_mcp_stdio_process_uses_trusted_workspace(tmp_path: Path) -> None:
    ...
    try:
        ...
        resumed = await app.command("approve", {
            "thread_id": app.session_id,
            "decisions": [{"type": "approve"}], "grants": [],
        })

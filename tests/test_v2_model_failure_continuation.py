...
from sayacode.app import _final_text
...
async def test_model_retry_exhaustion_is_a_failed_graph_run(tmp_path: Path) -> None:
    ...
    async with await AgentRuntime.open(tmp_path / "state") as runtime:
        handle = runtime.build_agent(profile(retries=1), [], context=context(tmp_path),
...
async def test_explicit_truncation_continues_within_model_budget(tmp_path: Path) -> None:
    ...
    async with await AgentRuntime.open(tmp_path / "state") as runtime:
        handle = runtime.build_agent(profile(), [], context=context(tmp_path),
                                     model_override=model)
...
async def test_truncation_stops_when_model_budget_is_exhausted(tmp_path: Path) -> None:
    model = SequenceModel(responses=[
        AIMessage(content="incomplete", response_metadata={"stop_reason": "max_tokens"})
    ])
    async with await AgentRuntime.open(tmp_path / "state") as runtime:
        handle = runtime.build_agent(profile(max_calls=1), [], context=context(tmp_path),
                                     model_override=model)

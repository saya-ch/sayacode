...
from sayacode.commands import BUILTIN_COMMANDS, CommandRouter
...
async def test_help_unknown_command_gives_discovery_hint(
    tmp_path, language: str, expected: str,
) -> None:
    router = CommandRouter(object(), tmp_path, PromptPreferences(language=language))
...
async def test_team_help_explains_background_approval(
    tmp_path, language: str, approval_word: str,
) -> None:
    router = CommandRouter(object(), tmp_path, PromptPreferences(language=language))

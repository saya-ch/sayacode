...
from sayacode.cli import _pending_team_approval, _resume_approval_from_terminal, amain
...
async def _app(tmp_path: Path, model: BaseChatModel, *, session_id: str = "session-one") -> SayacodeApp:
...
async def test_native_v3_model_stream_exposes_only_visible_text(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert all("message-start" not in delta and "content-block" not in delta for delta in deltas)
...
async def test_native_v3_tool_error_is_not_reported_as_start(tmp_path: Path) -> None:
    ...
    try:
        ...
        failed = public[-1:]
        assert failed == [{
            "type": "tool.failed", "thread_id": "tool-error", "tool_call_id": "boom-1",
            "tool_name": "explode", "error": "boom",
        }]
    finally:
        await app.aclose()
...
@pytest.mark.asyncio
async def test_mixed_approval_affects_only_approved_call_and_survives_reopen(tmp_path: Path) -> None:

...
from sayacode.terminal_ui import TerminalPresenter
...
async def test_positional_model_add_no_longer_calls_app(tmp_path: Path) -> None:
    ...
    result = await CommandRouter(
        App(), tmp_path, PromptPreferences(language="zh")
    ).dispatch("/model add fast openai gpt-fast")

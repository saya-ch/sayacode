...
from sayacode.cli.events import _exit_code, _with_task_outcome
...
@pytest.mark.parametrize(
    "verb,answers,expected_command,expected_decisions,expected_grants",
    [
        (
            "approve",
            ["s", "n"],
...
async def test_headless_team_approval_requires_interactive_input(tmp_path, monkeypatch, capsys):
    ...
    code = await amain(
        ["--workspace", str(tmp_path), "-p", "/team approve task-1", "--output-format", "jsonl"],
...
async def test_selected_session_restores_its_trust_without_cli_override(tmp_path, monkeypatch):
    ...
    await first.aclose()
    reopened = await create_app(
        build_parser().parse_args(["--workspace", str(tmp_path), "--session", session_id])
    )
    try:
...
@pytest.mark.asyncio
async def test_jsonl_observed_task_failure_overrides_parent_completion(
    ...
    events = [json.loads(line) for line in capsys.readouterr().out.splitlines()]
    assert code == 1
    assert [event["type"] for event in events] == ["run.started", "task.failed", "run.failed"]
...
module = importlib.import_module('sayacode.application')
class FakeApp:
    async def run(self, prompt, **kwargs):

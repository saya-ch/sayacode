...
| `host/` | 一个进程内协调器：工作区目录、共享运行时、任务句柄、排队与停止操作、本机附件、事件广播与产品操作。 |
...
LangGraph Store：工作区与会话目录、任务关系、Inbox 用户消息队列、交付和长期记忆
...
用户在运行中提交的消息先以唯一 ID 落入持久队列。普通排队在本轮结束后按顺序进入下一轮；“直接发送”把同一条消息改为下一步骤待送达，由模型调用前的中间件写入原生 `HumanMessage` 与检查点收据。已发出的模型请求和工具调用不会被改写。恢复和重复请求以消息 ID 与检查点收据去重；附件文件由本机目录保管，队列只保存受控引用。

主会话停止先阻止新派发和自动唤醒，再对主运行及所有后代任务发 `RunControl.request_drain()`。已保存检查点、排队消息和未应用 worktree 交付保留；进程重启不会因通知自动恢复停止的会话树。用户显式恢复时，若图还有待执行超步，先继续原图；否则处理排队消息。

本地审计保存调用 ID、耗时、用量等轨迹元数据。页面从原生 `AIMessage.tool_calls` 与 `ToolMessage` 补充工具参数、结果和失败状态，审计不保存第二份工具正文。长轨迹在前端按运行和调用 ID 合并、虚拟滚动。
...
全局模型配置定义连接与新会话的初始模型。主会话创建时记录当时选中的默认模型，此后切换全局默认不会改变已有会话；用户可为当前线程直接选择另一模型。子 Agent 默认保存派发时父线程的完整模型配置快照，也可为后续轮次单独选择另一模型。信任档同样按线程持久化。输入框内的模型、信任控件操作同一份线程元数据，完整设置窗口按全局、工作区和当前线程标明作用域。

<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/Version-3.2.1-EF5DA8" alt="版本 3.2.1">
...
**3.x 的运行方式：**交互式 TUI 已由 FastAPI + React 工作台取代；适合脚本和 CI 的无头 CLI 继续保留。3.1 增加运行中消息队列、整棵子 Agent 任务树停止和会话管理；3.2 整理四档信任策略；3.2.1 将工具与模型过程放入对话时间线。
...
运行中继续输入时，消息先进入输入框旁的持久队列。排队项可编辑、删除，或点击“直接发送”让同一条消息尽快在 Agent 的下一模型步骤生效；已经发出的模型请求和正在执行的工具不会被这个操作中途改写。输入框为空时，右下角的发送位置会按会话状态显示停止或恢复；停止主会话会同时请求所有子 Agent 停止，保留检查点和未处理交付，待用户显式继续。

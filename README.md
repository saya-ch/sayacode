...
The setup wizard asks for six fields: **API protocol**, **base URL**, **API key**, **model ID**, **context length**, and **maximum output tokens**. Choose the protocol your endpoint actually implements: `openai_chat_completions`, `openai_responses`, `anthropic_messages`, `gemini_generate_content`, or `ollama_native_chat`. For example, an OpenAI Chat Completions endpoint may use `https://api.openai.com/v1` as its base URL. The protocol determines the LangChain integration and request format; changing the URL alone does not convert one protocol into another. SAYACODE does not infer a protocol from a company or model name.

`/model add` stores a profile in `~/.sayacode/config.json` and generates its local name from the model ID. **The API key is required by default.** Hosted endpoints normally require a valid key and return HTTP 401 for a missing or invalid key. The wizard rejects a blank key and never reads an environment variable. Only for an endpoint that truly needs no authentication, enter `none` at the hidden key prompt. The key is stored in the local configuration file, and hidden input is excluded from terminal input history. To correct an existing profile, enter `/model key <name>` and supply the real key in the hidden prompt; `none` explicitly clears it for an unauthenticated endpoint. Do not put the key on the slash-command line. Use `/models` to see configured models and protocols, `/model use <name>` to switch, and `/model test [name]` to check text, tool-call, and streaming capabilities. `--profile <name>` selects a saved profile at launch. A minimal configuration file looks like this:
...
For a one-run override, supply `--protocol`, `--base-url`, `--model-id`, `--context-length`, and `--max-output-tokens` together, plus **either** `--api-key <key>` **or** `--no-api-key`. The two authentication flags are mutually exclusive; use `--no-api-key` only for an endpoint that needs no authentication. Both token counts accept values such as `128000`, `256k`, and `1M`. Shell command arguments can be visible to other local processes, so prefer a saved profile for credentials. A keyless profile gives the official SDK a harmless placeholder where required rather than reading a possibly sensitive provider key from the environment. The Agent passes tool schemas through the selected LangChain adapter. Optional official LLM tool selection can be enabled by setting `tool_selector_max_tools` to a positive number in a profile when the endpoint supports structured output; this adds another model call. The configured context length informs automatic summary timing (and configures `num_ctx` for Ollama). The setup supports the five listed wire protocols and explicit API-key authentication; an arbitrary new protocol or custom authentication scheme requires another official adapter or additional configuration.
...
`/team stop`, `/team resume`, and `/team followup <task-id> <message>` are also available. A task paused for approval can be inspected with `/team pending <task-id>` and handled interactively with `/team approve <task-id>` or `/team reject <task-id>`; the terminal asks for a decision on each pending action. The model can delegate a task and use `task_status`, `task_wait` (bounded wait), and `task_delivery` to inspect it. When a child completes, fails, pauses, or stops, its parent Agent is notified and starts a new run on the same LangGraph thread after any active parent run finishes. The notification supplies a task ID and status through run context; it does not fabricate a user message or copy the child transcript. The parent can call `task_status` for the result. If the parent run requests approval, use `/approve [session-id]` or `/reject [session-id]` in the terminal. Notifications are checkpoint-aware and are not blindly replayed after an uncertain forced interruption.

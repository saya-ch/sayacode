...
TOPICS = (
    ...
    HelpTopic(
        ...
        "work",
        "管理跨会话记忆、使用与自动学习",
        "Manage cross-session memories, retrieval, and learning",
        "/memory [操作] [参数]",
        "/memory",
        "直接输入 /memory 用方向键浏览、固定、纠正和遗忘。/memory recent 显示本轮已提供给模型的记忆及命中原因；提供不等于模型实际采用。/memory enable|disable、use <on|off>、learn <off|explicit|auto> 修改全局默认；/memory model <已配置画像|default> 选择记忆整理模型，default 跟随主模型；/memory timeout <秒> 设置无交互整理宽限期。/memory session 查看当前会话的实际设置，session use <on|off|default> 和 session learn <auto|explicit|off|default> 只影响当前会话。其他子命令：status、list、search <查询>、show <ID>、remember [user|project] <文本>、correct <ID> <文本>、confirm <ID>、pin <ID>、unpin <ID>、forget <ID>、refresh [ID]。固定不免除项目事实核验。人工说明保存在 SAYACODE_HOME/instructions.md、SAYACODE.md 或 CLAUDE.md；遗忘长期记忆不会删除原始聊天。",
        "Enter /memory to browse, pin, correct, and forget with arrow keys. /memory recent shows memories provided to the model this turn and why they matched; provision does not prove the model used them. /memory enable|disable, use <on|off>, and learn <off|explicit|auto> change global defaults. /memory model <saved-profile|default> chooses a memory learning model; default follows the main model. /memory timeout <seconds> sets the headless learning grace period. /memory session shows effective settings for this session; session use <on|off|default> and session learn <auto|explicit|off|default> affect only this session. Other actions: status, list, search <query>, show <ID>, remember [user|project] <text>, correct <ID> <text>, confirm <ID>, pin <ID>, unpin <ID>, forget <ID>, and refresh [ID]. Pinning never skips project fact verification. Manual instructions live in SAYACODE_HOME/instructions.md, SAYACODE.md, or CLAUDE.md. Forgetting a memory does not delete source conversations.",
        usage_en="/memory [action] [arguments]",
        example_en="/memory",
        quick_actions=(
            "/memory recent",
            "/memory list",

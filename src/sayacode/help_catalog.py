...
TOPICS = (
    ...
    HelpTopic("models", (), "model", "列出模型、接口协议和上下文配置", "List models, API protocols, and context limits", "/models", "/models", "用 /model add 打开向导，再用 /model use <名称> 切换。", "Use /model add to open setup, then /model use <name> to switch."),
    HelpTopic("model", ("config",), "model", "添加、切换和验证模型", "Add, switch, and verify models", "/model [list|show|add|key <名称>|use <名称>|remove <名称>|test [名称]]", "/model add", "在交互终端输入 /model add，依次填写接口协议、完整接口地址、API Key、模型 ID、上下文长度和最大输出。API Key 默认必填；仅无认证接口输入 none。留空会重新提示，也不会读取环境变量。已保存的配置用 /model key <名称> 在隐藏输入框中更新密钥；输入 none 可清除。密钥不进入命令历史；配置名自动生成。/model test [名称] 检查文本、工具调用和流式输出能力。", "Enter /model add in the interactive terminal to choose an API protocol and enter the full endpoint URL, API key, model ID, context length, and maximum output. The API key is required by default; enter none only for an unauthenticated endpoint. Blank prompts again and never reads an environment variable. Use /model key <name> to update a saved profile through a hidden prompt, or enter none to clear it. Keys are omitted from command history; a profile name is generated automatically. /model test [name] checks text, tool-call, and streaming capabilities.", usage_en="/model [list|show|add|key <name>|use <name>|remove <name>|test [name]]", quick_actions=("/model add",)),
    HelpTopic("trust", (), "model", "设置当前会话信任档位或新会话默认值", "Set session trust or the default for new sessions", "/trust [read_only|ask|full|default <档位>|clear]", "/trust ask", "只读档自动允许读取，Shell 仍逐次审批；询问档对每次有副作用的调用审批，可记住完全相同的调用；完全信任不弹审批。所有档位均无操作系统沙箱，绝对路径可访问工作区外。", "Read-only trust allows reads and asks for every Shell command; ask trust pauses every side-effecting call and can remember the exact call; full trust skips approvals. None provides an OS sandbox, and absolute paths may reach outside the workspace.", usage_en="/trust [read_only|ask|full|default <level>|clear]"),
    ...
    HelpTopic("prefs", (), "model", "查看语言与风格", "Show language and style", "/prefs", "/prefs"),
    ...
    HelpTopic("todos", (), "work", "显示当前线程待办", "Show the current thread's todos", "/todos", "/todos"),
    ...
    HelpTopic("approve", ("reject",), "work", "处理主 Agent 的待批准操作", "Review pending main-agent actions", "/approve [会话 ID] 或 /reject [会话 ID]", "/approve", "子任务完成后主 Agent 会自动继续；若继续过程中请求工具审批，用 /approve 查看并逐项决定。/reject 拒绝全部操作。", "A child result can start a parent turn. If that turn pauses for tool approval, use /approve to review each action or /reject to decline all.", usage_en="/approve [session ID] or /reject [session ID]"),
    ...
    HelpTopic("git", (), "work", "查看 Git 状态、差异与历史", "Read Git status, diff, and history", "/git [status|diff [ref]|log [数量]|branch|remote|show [ref]]", "/git diff", "Git 工具只执行查询；提交、拉取和推送等变更通过 Shell 执行，并按当前信任档位处理。", "The Git tool only queries; commits, pulls, pushes, and other changes use Shell under the current trust level."),

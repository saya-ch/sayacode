...
TOPICS = (
    ...
    HelpTopic("model", ("config",), "model", "添加、切换和验证模型", "Add, switch, and verify models", "/model [list|show|add|use <名称>|remove <名称>|test [名称]]", "/model add", "在交互终端输入 /model add，依次填写接口协议、完整接口地址、API Key（可空）、模型 ID、上下文长度和最大输出。密钥输入隐藏，不进入命令历史；配置名自动生成。/model test [名称] 检查文本、工具调用和流式输出能力。", "Enter /model add in the interactive terminal to choose an API protocol and enter the full endpoint URL, optional API key, model ID, context length, and maximum output. The key is hidden and omitted from command history; a profile name is generated automatically. /model test [name] checks text, tool-call, and streaming capabilities.", usage_en="/model [list|show|add|use <name>|remove <name>|test [name]]", quick_actions=("/model add",)),

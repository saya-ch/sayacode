...
2. 提供 BaseModel：给自有传输实现（不走 LangChain 集成的模型，
   以及测试替身）使用的抽象基类。

绝大多数模型不再继承本类。它们由 lib.models.providers 里的薄类提供，
直接多重继承 LangChain 的具体 chat model 与 ~lib.models.extras.ModelExtras。
...
class BaseModel(ModelExtras, ABC):
    ...
    def validate_temperature(self, temperature: float) -> float:
        """ModelExtras.clamp_temperature 的兼容别名。

        只在本类提供。LangChain 支撑的模型无法暴露这个名字：temperature
